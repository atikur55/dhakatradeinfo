    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
            	@if($subcategory->status==1)
                <a href="{{route('admin.subcategory.status', ['id' => $subcategory->id])}}" class="btn btn-success">Active</a>
                @else
				<a href="{{route('admin.subcategory.status', ['id' => $subcategory->id])}}" class="btn btn-danger">Deactive</a>
                @endif
...
<div class="modal fade" id="deleteModal{{$subcategory->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLongTitle" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <a href="{{route('admin.subcategory.delete', ['id' => $subcategory->id])}}" class="btn btn-danger">Delete</a>
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
            </div>
        </div>
    </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{route('admin.subcategory.update')}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                    	<label>Category Name:</label><br>
						<select style="width: 100%;" id="editid" class="form-control" name="category_id">
						    <option value="">Select</option>
						    @foreach($all_category as $category)
                    ...
                    <div class="form-group">
							...
							<div class="custom-file">
								<input type="file" class="custom-file-input" id="customFile" name="image" id="profile-img" onchange="preview_image(event)"/>
								<label class="custom-file-label" for="customFile">Choose file</label>
								<img src="{{asset('uploads/subcategory')}}/{{$subcategory->image}}" id="output_image" width="200px" style="padding-top: 5px;" />
							</div>
					</div>
					@error('image')
					   <div class="alert alert-danger">

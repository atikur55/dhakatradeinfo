                                    ...
                                    <div class="row">
                                        <div class="col-5">
                                            <div class="categories-details-img">
                                                <img class="img-fluid" src="{{ asset('uploads/subcategory') }}/{{ $subcategory->image??'' }}" alt="">
                                            </div>
                                        </div>
                                        <div class="col-7">
                                            <div class="categories-details">
                                                <h6><a href="{{ route('subcategory_product',['id'=>$subcategory->id]) }}">{{ $subcategory->title??'' }}</a></h6>
                                                @php
                                                    $child_categories = App\Models\ChildCategory::where('subcategory_id',$subcategory->id)->where('status',0)->get();
                                                @endphp
                                                <ul>
                                                    @forelse ($child_categories as $category)
                                                        <a href="{{ route('ch-products',['id'=>$category->id]) }}"><li>{{ $category->title??'' }}</li></a>
                                                    @empty
                                                        <li>No Data Found</li>
                                                    @endforelse
                                                </ul>
